...
| RTX A6000 (Ampere) | 48 GB | ✅ 완전 호환, 더 높은 VRAM |
...
각 릴리즈는 두 가지 압축파일로 제공됩니다:

| 압축파일 | CUDA 런타임 | 추천 대상 |
|---------|:------------:|----------|
| `ninfer-sm86-cuda12.zip` | CUDA 12.8+ | 기존 CUDA 12.x 환경 사용자 |
| `ninfer-sm86-cuda13.zip` | CUDA 13.x | 최신 드라이버 또는 CUDA 13.x 사용자 |

두 압축파일 모두 **sm_86**(Ampere)용 동일 바이너리를 포함합니다. 시스템 CUDA 메이저 버전과 일치하는 아카이브를 선택하세요. CUDA 13 변형은 MSVC의 conforming 전처리기를 사용합니다.
